...
class t6Sim {
  ...
  async simulate() {
    ...
    this.result = createResult(this, this.strat === "T6snax" ? logToExp(<number>this.stopC12[0], 2) : "");
    return this.result;
  }
  tick() {
  ...
  buyVariables() {
    if (this.stratIndex !== 12)
      ...
      while (true) {
        ...
        if (minCost[1] !== -1 && rawCost[minCost[1]] <= this.rho) this.variables[minCost[1]].buy();
        else break;
      }
    }
  }
